...
async def test_mock_claude_client_error_count(mock_claude_client, claude_manager):
    ...
    with pytest.raises(APIStatusError, match="Simulated API error") as excinfo:
        await claude_manager.generate_response("Test prompt", "claude-3-haiku-20240307")
    logger.info("APIStatusError raised as expected", error_message=str(excinfo.value))
    with pytest.raises(APIStatusError):
        await claude_manager.generate_response("Test prompt", "claude-3-haiku-20240307")
...
async def test_concurrent_claude_api_calls(mock_claude_client):
    ...
    logger.debug(f"Successful calls: {successful_calls}")
    logger.debug(f"Rate limit errors: {[str(e) for e in rate_limit_errors]}")
